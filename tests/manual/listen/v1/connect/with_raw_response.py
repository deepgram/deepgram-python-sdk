...
print("Starting listen v1 connect with raw response example script")
load_dotenv()
...
try:
    model = "nova-3"
    print(f"Establishing connection with raw response, model: {model}")
    with client.listen.v1.with_raw_response.connect(model=model) as connection:
