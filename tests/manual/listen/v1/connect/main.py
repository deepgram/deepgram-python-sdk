...
print("Starting listen v1 connect example script")
load_dotenv()
...
bit_rate = 705600  # bps
print(f"Audio properties - Sample rate: {sample_rate} Hz, Bit rate: {bit_rate} bps")

# Calculate chunk size for 250ms of audio (linear16 PCM: 2 bytes per sample)
# Assuming mono audio: bytes_per_second = sample_rate * 2
chunk_duration_ms = 1000  # 1s chunks
chunk_size = int(sample_rate * 2 * (chunk_duration_ms / 1000.0))
...
try:
    model = "nova-3"
    print(f"Establishing connection with model: {model}")
    with client.listen.v1.connect(model=model) as connection:
        ...
        for i in range(0, len(audio_data), chunk_size):
            ...
            if chunk:
                connection._send(chunk)  # type: ignore[attr-defined]
                chunk_count += 1
