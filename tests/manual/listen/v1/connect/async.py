...
print("Starting async listen v1 connect example script")
load_dotenv()
...
async def main() -> None:
    try:
        model = "nova-3"
        print(f"Establishing async connection with model: {model}")
        async with client.listen.v1.connect(model=model) as connection:
