...
# All 10 are auto-generated by Fern — we never modify their source.
#
# This list must cover every generated module with a websocket ``connect()``.
# A surface missing here fails silently: ``transport_factory`` is accepted, no
# error is raised, and the client connects to Deepgram Cloud instead of the
# custom transport. When a regen adds a new websocket surface, add it here.
# ---------------------------------------------------------------------------
_TARGET_MODULES = [
