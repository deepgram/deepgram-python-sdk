...
class SpeakV2Warning(UncheckedBaseModel):
    ...
    Inline-control codes. The offending control is dropped and the rest of the turn is synthesized: `PRONUNCIATION_WARNINGS` (one or more pronunciation overrides contained invalid IPA; the server falls back to a best-effort pronunciation and the `description` lists each offending sequence), `PRONUNCIATION_TOO_LONG` (an IPA string exceeded the length limit), `PRONUNCIATIONS_LIMIT_EXCEEDED` (too many pronunciation controls in one turn), `BREAKS_LIMIT_EXCEEDED` (too many pause controls, or two pauses with no intervening text), `BREAK_TOKENS_OUT_OF_RANGE` (pause durations outside the range the model supports), `BREAK_TOKENS_WITH_INVALID_INCREMENTS` (pause durations off the model's supported increment).
