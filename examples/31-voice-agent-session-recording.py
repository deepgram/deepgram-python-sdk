...
voice-agent-session.json after the session ends. Those fields can contain
sensitive data. Apply your application's consent, redaction, retention, and
storage policies before using this pattern in production.
...
OUTPUT_PATH = Path("voice-agent-session.json")
...
def main() -> None:
    ...
    try:
        ...
        recorder.finish()
        OUTPUT_PATH.write_text(json.dumps(recorder.to_dict(), indent=2), encoding="utf-8")
        print(f"Wrote session record to {OUTPUT_PATH}")
