        ...
        #     print(f"Request Argument - {key}: {value}")
            try:
                ...
                if isinstance(e, httpx.ConnectTimeout):
                    retries += 1
                    # print(f"Request timed out (Retry {retries + 1}/{max_retries}): {e}")
                    continue
                if isinstance(e, httpx.HTTPStatusError):
                    status_code = e.response.status_code or 500
                    if is_json(e.response.text):
                        json_object = json.loads(e.response.text)
                        raise DeepgramApiError(json_object.get('err_msg'), status_code, json.dumps(json_object)) from e
                    else:
                        raise DeepgramUnknownApiError(e.response.text, status_code) from e
                else:
                    raise
            except Exception as e:
                raise DeepgramUnknownError("An unknown error occurred during the request.", e) from e
        raise DeepgramUnknownError(f"Failed to make the request after {max_retries} retries.", "timeout error")