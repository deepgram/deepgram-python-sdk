    ...
    async def _process_text(self, message: str) -> None:
        ...
        try:
            ...
            match response_type:
                ...
                case AgentWebSocketEvents.EndOfThought:
                    end_of_thought_result: EndOfThoughtResponse = (
                        EndOfThoughtResponse.from_json(message)
                    )
                    self._logger.verbose(
                        "EndOfThoughtResponse: %s", end_of_thought_result
                    )
                    await self._emit(
                        AgentWebSocketEvents(AgentWebSocketEvents.EndOfThought),
                        end_of_thought=end_of_thought_result,
                        **dict(cast(Dict[Any, Any], self._kwargs)),
                    )
