...
- `src/deepgram/transport_interface.py`, `src/deepgram/transport.py`, `src/deepgram/transports/` — custom transport layer. **`_TARGET_MODULES` in `transport.py` must list every generated module with a websocket `connect()`.** A regen that adds a new websocket surface must add it here; a missing surface fails silently (`transport_factory` is accepted, nothing raises, and the client connects to Deepgram Cloud instead of the custom transport). This happened once: `speak.v2` landed in the 2026-07-08 regen and was absent until 2026-08-14, which meant Flux TTS could not run on SageMaker at all. `TestTargetModuleCoverage` in `tests/custom/test_transport.py` now scans the package and fails if the list is incomplete or stale
