...
- `src/deepgram/types/deepgram.py`, `src/deepgram/requests/deepgram.py`, `src/deepgram/types/speak_v2speed.py`, `src/deepgram/types/speak_v2expressivity.py` — spec **copy** fixes mirrored by hand from [deepgram-docs#1094](https://github.com/deepgram/deepgram-docs/pull/1094) so the 2026-08-12 Flux TTS launch does not ship stale text, without needing a generator re-run. Removes the Flux TTS "Early Access" caveats (Flux is GA; the direct Speak V2 path had already been updated, leaving the Agent speak-provider path as the only surface still claiming EA), and states the accepted values for `speed`/`expressivity` — both are enums in the spec, but Fern drops numeric enums, so the generated `speed` docstring pointed at a list that wasn't there and `expressivity` gave no range at all. **Unfreeze all four at the next regen**: once #1094 is merged the generator emits this text itself, and these entries become pure maintenance burden.
